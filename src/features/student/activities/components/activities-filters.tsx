...
interface ActivitiesFiltersProps {
  ...
  onFilterChange: (filters: ActivitiesFilters) => void
}

export function ActivitiesFilters({ filters, onFilterChange }: ActivitiesFiltersProps) {
  const [isOpen, setIsOpen] = React.useState(false)

  const statuses = ['All Status', 'Approved', 'Review', 'Pending', 'In Progress', 'Done', 'Resubmit', 'Feedback']
  const grades = ['All Grades', 'A+', 'A', 'B+', 'B', 'C+', 'C', 'D+', 'D', 'F']
          ...
          <div className="absolute right-0 top-full mt-2 w-80 bg-white rounded-xl border border-[hsl(214,32%,91%)] shadow-lg z-50 animate-fadeIn">
            ...
            <div className="p-4 space-y-4">
              ...
              {/* <div className="space-y-3">
                <label className="block text-xs font-medium text-[hsl(220,9%,46%)] uppercase tracking-wider">
                  Date Range
                </label>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-[hsl(220,9%,46%)] mb-1">
                      From Date
                    </label>
                    <input
                      type="date"
